...
malpolon_df = pd.read_csv(os.path.join(inp_dir, "malnutrition_dataset_malpolon.csv"))
k = np.random.randint(n_dhs_data)
survey_id = malpolon_df["survey_id"][k]
x_11x11 = np.load(os.path.join(inp_dir, "modality_11x11", "%s.npy" % survey_id))
x_21x21 = np.load(os.path.join(inp_dir, "modality_21x21", "%s.npy" % survey_id))
x_srtm = np.load(os.path.join(inp_dir, "modality_srtm", "%s.npy" % survey_id))
